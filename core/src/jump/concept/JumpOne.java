...
import jump.concept.data.SquareEntity;
import jump.concept.data.InputState;
import jump.concept.geometry.Direction;
import jump.concept.input.Keyboard;

public class JumpOne extends ApplicationAdapter {
	//GDX specific
	BitmapFont font;
	SpriteBatch batch;
	...
	public static class GameState {
		...
		public SquareEntity player =
				new SquareEntity(150, 150, 100, 0, 350);
	}
	...
	public static void applyPlayerInput(InputState inputState, GameState state, float delta) {
		if (inputState.keyboardKeyState.get(Keyboard.SPACE) == Boolean.TRUE) {
	...
	private int halfSide = 100/2;
	private int jumpFpsCounter = halfSide;
	...
	public void render () {
		...
		if (state.player.jumping) {
			jumpFpsCounter = jumpFpsCounter + direction * 10;
			if (jumpFpsCounter == halfSide * 8 && direction == 1) {
				direction = -1;
			} else if (jumpFpsCounter == halfSide && direction == -1) {
				direction = 1;
